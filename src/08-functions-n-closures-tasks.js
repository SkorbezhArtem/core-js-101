...
function getPolynom(...args) {
  return (curE) => args.reduce((acc, curr, i) => acc + curr * curE ** (args.length - i - 1), 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const obj = JSON.stringify(args).replace(/^.|.$/g, '');
    logFunc(`${func.name}(${obj}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${obj}) ends`);
    return res;
  };
}
